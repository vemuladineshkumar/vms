...
$pass ="SELECT `password` FROM registration WHERE username='$name'";
...
if($conn){
	...
	if (mysqli_num_rows($result) == 0) {
		//select
        echo "<script>alert('Username $username is not registered. Please tyr signup')</script>";
        echo "<script>window.open('signupfrontend.php','_self')</script>";
         }
